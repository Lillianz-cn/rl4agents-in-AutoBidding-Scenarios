...
from auction_sim.utils import ResultVisualizer  # 新增的导入
    ...
    try:
        ...
        for agent in agents:
            if not hasattr(agent, 'history'):
                ...
                # 过滤无效记录
                agent.history = [r for r in agent.history if isinstance(r, dict)]
        
        # 生成图表（假设前200轮是热身期）
        visualizer = ResultVisualizer()
        visualizer.save_visualizations(agents, warmup_rounds=200)
